...
const ViewerControls: React.FC<ViewerControlsProps> = ({
  ...
  const startViewing = async () => {
    ...
    try {
      ...
      const response = await sendWhepOfferWithRetry(whepUrl, currentSdp, streamIdToUse)
      ...
      if (response.ok) {
        ...
        if (resilienceManager && streamIdToUse) {
          const iceRestartEndpoint = getIceRestartEndpointBase()
          resilienceManager.updateIceRestartConfig(iceRestartEndpoint, streamIdToUse)
        }
  ...
  const sendWhepOfferWithRetry = async (url: string, sdp: string, streamIdToUse: string, maxRetries = 3): Promise<Response> => {
  ...
  const collectViewerStats = async (pc: RTCPeerConnection) => {
    try {
      ...
      setCurrentStats({ bitrate, fps, resolution, latency })
