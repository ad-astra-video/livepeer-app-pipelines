...
from ...globals import execution_providers, map_faces, source_path, target_path, many_faces, mouth_mask, show_mouth_mask_box, color_correction, source_target_map, simple_map, mask_down_size, mask_size, mask_feather_ratio
from . import core
from ...core import update_status
from ...face_analyser import get_one_face, get_many_faces, default_source_face
...
from ...cluster_analysis import find_closest_centroid
...
def pre_start() -> bool:
    if not map_faces and not is_image(source_path):
        update_status("Select an image for source path.", NAME)
        return False
    elif not map_faces and not get_one_face(
        cv2.imread(source_path)
    ):
        update_status("No face in source path detected.", NAME)
        return False
    if not is_image(target_path) and not is_video(
        target_path
    ):
...
def swap_face(source_face: Face, target_face: Face, temp_frame: Frame) -> Frame:
    ...
    if mouth_mask:
        ...
        mouth_mask_result, mouth_cutout, mouth_box, lower_lip_polygon = (
        ...
        if show_mouth_mask_box:
            mouth_mask_data = (mouth_mask_result, mouth_cutout, mouth_box, lower_lip_polygon)
...
def process_frame(source_face: Face, temp_frame: Frame) -> Frame:
    ...
    if many_faces:
        many_faces_result = get_many_faces(temp_frame)
        if many_faces_result:
            for target_face in many_faces_result:
        ...
        if many_faces:
            ...
            if detected_faces:
                if len(detected_faces) <= len(
                    ...
                    i = 0
                    for target_embedding in simple_map[
                        "target_embeddings"
                    ]:
                        closest_centroid_index, _ = find_closest_centroid(
